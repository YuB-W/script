...
local function scanDescendantsSlowly(root, callback, batchSize, onComplete)
    batchSize = batchSize or 20
    local all = root:GetDescendants()
    local total = #all
    local idx = 1

    -- Scan in a coroutine, yielding every batch
    coroutine.wrap(function()
        while idx <= total do
            for i = idx, math.min(idx + batchSize - 1, total) do
                local v = all[i]
                callback(v)
            end
            idx = idx + batchSize
            task.wait() -- let Roblox breathe
        end
        if onComplete then onComplete() end
    end)()
end

-- Example: Use for Recoil and MaxSpread
local function setNoRecoil()
    local weapons = game.ReplicatedStorage:FindFirstChild("Weapons")
    if not weapons then return end

    scanDescendantsSlowly(weapons, function(v)
        if v.Name == "RecoilControl" and v:IsA("NumberValue") and v.Value ~= 0 then
            v.Value = 0
        elseif v.Name == "MaxSpread" and v:IsA("NumberValue") and v.Value ~= 0 then
            v.Value = 0
        end
    end, 20) -- 20 per frame; adjust for faster/slower
end

Recoil.MouseButton1Down:Connect(setNoRecoil)
...
FFSpeed.MouseButton1Down:Connect(function()
    ...
    task.spawn(function()
        ...
        while running do
            local batchSize = 25 -- Number of items per batch before yielding
            local count = 0

            for _, v in ipairs(weapons:GetDescendants()) do
                if v.Name == "Auto" and v:IsA("BoolValue") and not v.Value then
                    v.Value = true
                elseif v.Name == "FireRate" and v:IsA("NumberValue") and v.Value ~= 0.02 then
                    v.Value = 0.02
                end

                count = count + 1
                if count % batchSize == 0 then
                    task.wait() -- Yield to avoid lag/freeze
                end
            end

            task.wait(10) -- Wait 5 seconds before next update
        end
    end)
end)
